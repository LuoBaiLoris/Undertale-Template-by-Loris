...
{		
	...
	//Choice.text = "{font:font_eng}{*}{noskip}{novoice}{pn:100}"+Chosen.Actions[0].Name
}
State_Start = false
...
if Input_Down_Pressed()
{
	Select_ ++
	Select ++
	if Select<=array_length(Chosen.Actions)-1 audio_play_sound(mmove,0,0);
	if Select_ > 2
	{
...
Select = clamp(Select,0,array_length(Chosen.Actions)-1)
Select_ = clamp(Select_,0,clamp(array_length(Chosen.Actions),1,3)-1)
if y_target < -(array_length(Chosen.Actions)-1)+2
{
	y_target = -(array_length(Chosen.Actions)-1)+2
}
if y_target > 0
{
	y_target = 0
...
next = STATE.ActMenuDialog
if Input_Confirm_Pressed()
{
	//Chosen = Chosen.Actions[Select]
